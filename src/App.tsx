  ...
  return (
    <main>
      ...
      <section className="intro" id="experience">
        <div>
          <div className="section-kicker">The Grandview experience</div>
      ...
      <section className="feature-grid" id="dining">
        <article className="feature-card large feature-room">
          <div className="feature-content"><span>01 / ROOMS</span><h3>Quiet luxury, made practical.</h3><a href="#rooms">View rooms →</a></div>
        </article>
        <article className="feature-card feature-food">
          <div className="feature-content"><span>02 / DINING</span><h3>Local flavour, modern table.</h3><a href="#dining">Discover dining →</a></div>
        </article>
        <article className="feature-card feature-service">
          <div className="feature-content"><span>03 / SERVICE</span><h3>Hospitality that remembers the details.</h3></div>
        </article>
      </section>
      ...
      <section className="rooms-section" id="rooms">
        <div className="rooms-heading">
          ...
          <span className="availability">● Live availability</span>
        </div>
          ...
          {rooms.map((room) => (
            <article className="room-card" key={room.id}>
              ...
              <div className="room-body">
                <div className="room-topline"><span>{room.size}</span><span>Up to {room.capacity} guests</span></div>
                <h3>{room.name}</h3>
                <p>{room.description}</p>
                <div className="room-tags">{room.amenities.map((item) => <span key={item}>{item}</span>)}</div>
                <div className="room-footer">
                  <div><strong>{formatCurrency(room.price)}</strong><span> / night</span></div>
                  <button onClick={() => openBooking(room)}>Reserve room</button>
                </div>
              </div>
            </article>
          ))}
      ...
      <section className="location-section" id="location">
        <div className="location-copy">
          <div className="section-kicker">Perfectly placed</div>
          <h2>Stay close to<br /><span>what matters.</span></h2>
          <p>Conveniently located for city business, shopping, dining and local travel. The hotel team can arrange cabs and airport transfers on request.</p>
          <div className="distance-list">
            <div><span>Railway Station</span><strong>10 min</strong></div>
            <div><span>Airport</span><strong>25 min</strong></div>
            <div><span>City Centre</span><strong>8 min</strong></div>
          </div>
        </div>
        <div className="map-card">
          <div className="map-grid" />
          <div className="map-pin">GRANDVIEW</div>
          <div className="map-label">Raipur, Chhattisgarh</div>
        </div>
      </section>

      <footer>
        <div className="footer-brand">GRANDVIEW RAIPUR</div>
        <p>Direct booking. Thoughtful stays.</p>
        <div className="footer-links"><a href="#top">Back to top ↑</a><span>© 2026 Grandview Hospitality</span></div>
      </footer>

      {bookingOpen && selectedRoom && (
        <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setBookingOpen(false)}>
          <div className="booking-modal">
            <button className="close" onClick={() => setBookingOpen(false)}>×</button>
            <div className="section-kicker">Reserve directly</div>
            <h2>{selectedRoom.name}</h2>
            <div className="booking-summary">
              <span>{checkIn} → {checkOut}</span>
              <strong>{nights} {nights === 1 ? 'night' : 'nights'} · {formatCurrency(selectedTotal)}</strong>
            </div>
            <form onSubmit={submitBooking}>
              <label>Full name<input value={guestName} onChange={(e) => setGuestName(e.target.value)} required placeholder="Your name" /></label>
              ...
              <button className="primary-btn full" disabled={loading}>{loading ? 'Confirming…' : 'Confirm booking'}</button>
            </form>
            <p className="modal-note">Prototype booking flow — payment can be connected in the next iteration.</p>
          </div>
        </div>
      )}
      ...
      {booking && (
        <div className="modal-backdrop">
          <div className="booking-modal success">
            ...
            <h2>You’re booked.</h2>
            <p>Your direct reservation has been created successfully.</p>
            <div className="confirmation">
              <span>Confirmation</span><strong>{booking.id}</strong>
              <span>Room</span><strong>{booking.roomName}</strong>
              <span>Stay</span><strong>{booking.checkIn} → {booking.checkOut}</strong>
              <span>Total</span><strong>{formatCurrency(booking.total)}</strong>
            </div>
            <button className="primary-btn full" onClick={() => setBooking(null)}>Done</button>
          </div>
        </div>
      )}
    </main>
